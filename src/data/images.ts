import { Header, Story } from "react-insta-stories/dist/interfaces";

const headerForAll: Header= 
{
  heading: 'Preguntas frecuentes',
  subheading: '',
  profileImage: 'https://res.cloudinary.com/dlbhecdey/image/upload/v1710445477/juh33eq1elbilnvkzqvt.png'
}

export const stories: Story[]= [
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/j4h7hplekiqqkjkpcv2t.jpg", header:headerForAll },
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/eqziju1oumoi4bp4koqy.jpg",header:headerForAll },
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/slnebyjnljsseylhfrv9.jpg", header:headerForAll },
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/aun4vnnqsalhyaq19esv.jpg", header: headerForAll },
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/ntwbsiqs1exsyeqotwut.jpg", header: headerForAll },
  { url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710366129/kbcctsosamttwlzskza9.jpg", header: headerForAll }
];


const header2: Header={
  heading: 'Sobre la autora',
  subheading: 'Concepción del Uruguay',
  profileImage: "https://res.cloudinary.com/dlbhecdey/image/upload/v1692319547/gvi24l3ams49u1jragst.png"
};

export const stories2: Story[]= [
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448968/sdtfv7uhd2xi2qhdgrih.jpg", header: header2},
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448969/lhytqumqe6m7vl7ahwl6.webp", header: header2},
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448970/jkysdwb9cslakjyanp22.webp", header: header2},
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448971/vehgv4jgaazor5ms4bqc.webp", header: header2},
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448969/dmjsecmfcpvepqcxaibn.webp", header: header2},
  {url:"https://res.cloudinary.com/dlbhecdey/image/upload/v1710448968/fip64nb7ctke9gpjuxyq.webp", header: header2}
]

const header3: Header={
  heading: 'Formato de serie',
  subheading: '4 tomos',
  profileImage: "https://res.cloudinary.com/dlbhecdey/image/upload/v1710445477/juh33eq1elbilnvkzqvt.png"
}

export const stories3: Story[] = [
  {url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1712629538/uxirueabb4ebhr8jo2x4.png", header: header3},
  {url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1712629538/naqfaeikilbqadvmhoun.png",  header: header3},
  {url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1712629539/yuwmbu5dijqsfficfaws.png",  header: header3},
  {url: "https://res.cloudinary.com/dlbhecdey/image/upload/v1712629540/yhxlh0niv1qufyxuk07z.png",  header: header3}
]